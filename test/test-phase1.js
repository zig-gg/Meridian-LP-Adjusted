...
// ── Group 22: Ops-9c Config Precedence — setup.js must not override 100M default ─
...
test("Ops-9c Test 4: user-config.example.json maxMcap is 100000000", () => {
  const content = readFileSync("user-config.example.json", "utf8");
  const parsed = JSON.parse(content);
  assert.strictEqual(parsed.maxMcap, 100_000_000,
    "user-config.example.json maxMcap must be 100000000");
});

test("Ops-9c Test 5: setup.js degen preset maxMcap remains at 5_000_000 (intentional low ceiling)", () => {
  const content = readFileSync("setup.js", "utf8");
  const degenIdx = content.indexOf("degen: {");
  assert.ok(degenIdx >= 0, "setup.js must define degen preset");
  const degenBlock = content.slice(degenIdx, content.indexOf("},", degenIdx) + 2);
  assert.ok(degenBlock.includes("5_000_000") || degenBlock.includes("5000000"),
    "setup.js degen preset maxMcap must remain at 5_000_000 (different risk profile)");
});

test("Ops-9c Test 6: setup.js passes node --check after Ops-9c edits", () => {
  execSync("node --check setup.js", { cwd: process.cwd(), stdio: "pipe" });
});
